...
describe('features.login', function () {
    ...
    xit('will give an error if you try to login without username and password', function () {
        return loginWorkflow(driver, '', '').sleep(10000)
        .waitForElementByCssSelector('#login-feature .alert', 10000);
    });

    xit('will give an error if you try to login with username and without password for single agency', function () {
        return loginWorkflow(driver, users.singleAgency.username, '').sleep(10000)
        .waitForElementByCssSelector('#login-feature .alert', 10000);
    });

     xit('will give an error if you try to login with username and without password for multi agency', function () {
        return loginWorkflow(driver, users.multiAgency.username, '').sleep(10000)
        .waitForElementByCssSelector('#login-feature .alert', 10000);
    });

    xit('will give an error if you try to login without username and with password for single agency', function () {
        return loginWorkflow(driver, '', users.singleAgency.password).sleep(10000)
        .waitForElementByCssSelector('#login-feature .alert', 10000);
    });

    xit('will give an error if you try to login without username and with password for multi agency', function () {
